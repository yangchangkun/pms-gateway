function loadReportData(day, proId, type){
    ...
    var url = "app/attend/report/attendReportProTeam";
    $.ajax({
        url: baseURL + url,
        type: "POST",
        ...
        success: function(r){
            ...
            if(r.code == 0){
                vm.day = r.day;
                vm.proBaseEntity = r.proBaseEntity;
                var list = r.proTeamAttendList;

                if(type=='0'){
                    vm.proTeamAttendList = list;
                } else {
                    if(list!=null && list.length>0){
                        for(var i=0;i<list.length;i++){
                            var staffAttend = list[i];
                            if(type=='1'){
                                //1：出差；2：请假；3：外出；4：外勤；5：迟到；6：旷工；7：未打卡；
                                if(!isBlank(staffAttend.travelTag)){
                                    vm.proTeamAttendList.push(staffAttend);
                                }
                            } else if(type=='2'){
                                //1：出差；2：请假；3：外出；4：外勤；5：迟到；6：旷工；7：未打卡；
...
var vm = new Vue({
                ...
                var iosSelect = new IosSelect(3,
                    [yckYearData, yckMonthData, yckDateData],
                    {
                        ...
                        callback: function (selectOneObj, selectTwoObj, selectThreeObj) {
                            ...
                            vm.day = day;
                            loadReportData(vm.day, vm.proId);
                        },
                        fallback:function(){
                            vm.day = currentDate();
                            loadReportData(vm.day, vm.proId);
    ...
    mounted: function () {
        ...
        this.proId = proId;
        printLog("proId="+proId);
        var type = urlParams.type;
        this.type = type;
        printLog("type="+type);

        loadReportData(day, proId, type);
