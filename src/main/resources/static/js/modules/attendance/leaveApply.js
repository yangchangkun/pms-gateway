...
function init(leaveDay){
    ...
    $.ajax({
        type: "POST",
        url: baseURL + "app/attend/leave/init",
        data: {},
        success: function(r){
            ...
            if(r.code == 0){
                ...
                vm.staffHolidayMap = r.staffHolidayMap;

                if (!isBlank(leaveDay)){
                    vm.entity.startTime = leaveDay+" 09";
                    vm.entity.endTime = leaveDay+" 18";
                }
...
var vm = new Vue({
    ...
    data: {
        ...
        staffHolidayMap:{
            ...
            useSickQuantityOfQuarter : 0,
            useSickQuantityOfCurrMonth : 0,
        },
         ...
         * 审批流程图的url
         */
        processInstanceImgUrl:baseURL + "app/common/act/getActivitiProccessDefinitionImage?key=attendance_leave",
    ...
    methods: {
        ...
        toList:function(){
            openWin("../attendance/leaveList.html");
        },
        ...
        showLeaveDetailDialog:function(){
            $('#modal-leave-valid-duration').modal("show");
        },
        ...
        datePickerChange: function(ele, fmt, tag, idx){
            ...
            if(tag=="entity.startTime"){
                var iosSelect = new IosSelect(4,
                    [yckYearData, yckMonthData, yckDateData, yckDutyHourData],
                    {
                        title: '时间选择',
                        itemHeight: 35,
                        ...
                        callback: function (selectOneObj, selectTwoObj, selectThreeObj, selectFourObj) {
                            var day = selectOneObj.value + '-' + selectTwoObj.value + '-' + selectThreeObj.value + ' ' + selectFourObj.value ;
                            vm.entity.startTime = day;
                        },
                        fallback:function(){
                            vm.entity.startTime = "";
                        }
                    });
            } else if(tag=="entity.endTime"){
                var iosSelect = new IosSelect(4,
                    [yckYearData, yckMonthData, yckDateData, yckDutyHourData],
                    {
                        title: '时间选择',
    ...
    mounted: function () {
        ...
        var leaveDay = urlParams.leaveDay;
        printLog("leaveDay="+leaveDay);
