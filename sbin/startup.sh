...
#定义启动的环境变量
DEV_ENV=(dev pro local test uat yx)
...
#JAVA_OPTS="-Dspring.profiles.active=$EVN -Duser.timezone=GMT+8 -server -Xms512m -Xmx1024m ${GC_OPTS} ${HEAP_DUMP}  -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5008 "
JAVA_OPTS="-Dspring.profiles.active=$EVN -Duser.timezone=GMT+8 -server -Xms512m -Xmx2048m ${GC_OPTS} ${HEAP_DUMP} "
...
#日志切割方案
splits(){
this_path=$(cd `dirname $1`;pwd)

cd $this_path
echo $this_path
current_date=`date -d "-1 day" "+%Y%m%d"`
echo $current_date
split -b 65535000b -d -a 4 /home/.../nohup.out   /home/.../log/log_${current_date}_
...
startup(){
    ...
    if [ $TPID -ne 0 ]; then
        ...
        nohup java $JAVA_OPTS -classpath $CLASSPATH $APPLICATION_MAIN >> $LOG_DIR/nohup.log 2>&1 &
        getPID
        if [ $TPID -ne 0 ]; then
